...
import { SNAP_THRESHOLD, SNAP_THRESHOLD_MOBILE, CANVAS_WIDTH, CANVAS_HEIGHT } from '../utils/constants';
...
export class DragHandler {
  ...
  private startDrag(pos: Vec2, targetId: string | null): void {
    ...
    eventBus.emit('drag:start', { x: pos.x, y: pos.y, targetId: targetId ?? '' });
  ...
  private moveDrag(pos: Vec2): void {
    ...
    eventBus.emit('drag:move', { x: pos.x, y: pos.y, dx: delta.x, dy: delta.y });
  ...
  private endDrag(pos: Vec2): void {
    ...
    const el = this.state.targetId ? this.elements.get(this.state.targetId) : null;
    el?.onDragEnd?.(pos);
  }
