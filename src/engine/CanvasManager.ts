...
export class CanvasManager {
	...
	resize(): void {
		...
		// Canvases are position:absolute (out of flow), so flex centering on the
		// container has no effect — offset each canvas explicitly instead.
		...
		for (const [, mc] of this.canvases) {
			...
			mc.canvas.style.left = `${offsetX}px`;
			mc.canvas.style.top = `${offsetY}px`;
		...
		log.info(
			`Resized: logical ${CANVAS_WIDTH}×${CANVAS_HEIGHT}, display ${displayW}×${displayH} @${dpr}x`,
		);
	}
