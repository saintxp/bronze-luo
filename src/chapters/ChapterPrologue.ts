...
export class ChapterPrologue extends ChapterBase {
	...
	private savedSwipeX = 0; // cumulative drag distance across multiple drags
	private readonly SWIPE_DISTANCE = 400; // px to complete the swipe
	...
	private setupMapDrag(): void {
		...
		this.dragHandler.onDrag((state) => {
			// Map absolute horizontal drag distance to swipe progress
		...
		this.dragHandler.onDragEnd(() => {
			// Save progress so the next drag continues from here
