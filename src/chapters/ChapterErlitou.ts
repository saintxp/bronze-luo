...
export class ChapterErlitou extends ChapterBase {
	...
	private onPuzzleSolved:
		| ((payload: { chapterId: string; puzzleId: string }) => void)
		| null = null;
	...
	enter(): void {
		...
		this.onPuzzleSolved = (payload) => {
			if (payload.chapterId !== "erlitou" || payload.puzzleId !== "tao-fan")
				return;
	...
	update(dt: number): void {
		...
		switch (this.frame) {
			...
			case ErlitouFrame.SOLVED_ANIM:
				this.solvedProgress = Math.min(
					1,
					this.frameTimer / this.SOLVED_DURATION,
				);
			...
			case ErlitouFrame.COMPLETE: {
				...
				const fadeAlpha = Math.min(
					1,
					this.frameTimer / this.TRANSITION_DURATION,
				);
				if (fadeAlpha >= 1) {
					this.completed = true;
	...
	private advanceToSolved(): void {
		...
		this.particles.trigger("copperSplash", SEAM_X, CY);
