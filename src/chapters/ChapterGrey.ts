...
import { SNAP_THRESHOLD, NEAR_THRESHOLD, CANVAS_WIDTH, CANVAS_HEIGHT } from '../utils/constants';
...
function makeTargets(): CrackTarget[] {
  return [
    ...
    { offset: { x: -120, y: 70 },  solved: false, grayValue: 120, holeRadius: 40 },
    { offset: { x: 100, y: -90 },  solved: false, grayValue: 100, holeRadius: 35 },
    { offset: { x: -80, y: 120 },  solved: false, grayValue: 75,  holeRadius: 50 },
...
export class ChapterGrey extends ChapterBase {
  ...
  private setupPuzzleDrag(): void {
    ...
    this.dragHandler.onDrag((state: DragState) => {
      ...
      this.panOffset.x += state.delta.x;
      this.panOffset.y += state.delta.y;
  ...
  private checkCrackAlignment(): void {
    ...
    if (dist < SNAP_THRESHOLD) {
      ...
      if (this.solvedCount >= this.targets.length) {
        eventBus.emit('puzzle:solved', { chapterId: 'grey', puzzleId: 'crack-nest' });
  ...
  private renderPuzzle(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.solvedCount < this.targets.length) {
      ...
      this.drawCrack(pCtx, curCX, curCY, cur.holeRadius, this.solvedCount * 7 + 3);
      pCtx.fill();
    }
    ...
    for (let i = this.solvedCount; i < this.targets.length; i++) {
      ...
      const pulse = isCurrent ? 0.7 + 0.3 * Math.sin(performance.now() / 600) : 1;
      const ghostAlpha = isCurrent ? 0.30 * pulse : 0.12;
      ...
      if (isCurrent) {
        const glow = pCtx.createRadialGradient(tx, ty, 0, tx, ty, t.holeRadius + 25);
    ...
    if (this.isNear && this.solvedCount < this.targets.length) {
      ...
      const curCX = CX + this.panOffset.x;
      const curCY = CY + this.panOffset.y;
      const glow = pCtx.createRadialGradient(curCX, curCY, 5, curCX, curCY, cur.holeRadius + 15);
      glow.addColorStop(0, 'rgba(200,166,90,0.25)');
      glow.addColorStop(1, 'rgba(200,166,90,0)');
      pCtx.fillStyle = glow;
      this.drawCrack(pCtx, curCX, curCY, cur.holeRadius + 10, this.solvedCount * 7 + 5);
      pCtx.fill();
    }
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'alphabetic';
    ctx.fillText(`裂缝 ${this.solvedCount}/${this.targets.length}`, CX, CANVAS_HEIGHT - 30);
