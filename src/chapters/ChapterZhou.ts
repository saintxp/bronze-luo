...
export class ChapterZhou extends ChapterBase {
	...
	private sundialSavedX = 0; // persistent drag offset across drags
	private tripodProgress = 0; // 0-1
	private tripodSavedX = 0;
	...
	enter(): void {
		super.enter();
		this.resetState();
		this.frame = ZhouFrame.SAL_SUN_DIAL;
	...
	private resetState(): void {
		...
		this.strikerBaseX = BELLS_START_X + RESONANCE_INDEX * BELL_SPACING;
		this.bellsAlpha = 0;
	...
	update(dt: number): void {
		...
		switch (this.frame) {
			...
			case ZhouFrame.LI_BELLS_APPEAR:
				...
				if (this.frameTimer >= this.AUTO_BELLS_APPEAR)
					this.advanceToTuneBells();
				break;
	...
	private setupSundialDrag(): void {
		...
		// 200px total drag = full phase sweep
		const PHASE_PX = 50;
		this.dragHandler.onDrag((state: DragState) => {
			const totalX = this.sundialSavedX + state.totalDelta.x;
			const rawPhase = Math.round(totalX / PHASE_PX);
		...
		this.dragHandler.onDragEnd(() => {
			this.sundialSavedX = this.sandialPhase * PHASE_PX;
		});
	}
	...
	private setupTripodDrag(): void {
		...
		this.dragHandler.attach(canvas);
		this.dragHandler.setMode("layer");
		// 300px total drag to fill the tripod
		const TRIPOD_DIST = 300;
		this.dragHandler.onDrag((state: DragState) => {
			const totalX = this.tripodSavedX + Math.abs(state.totalDelta.x);
			this.tripodProgress = Math.min(1, totalX / TRIPOD_DIST);
			if (this.tripodProgress >= 1) {
				this.advanceToKongLao();
			}
		});
		this.dragHandler.onDragEnd(() => {
			this.tripodSavedX = this.tripodProgress * TRIPOD_DIST;
		});
	}
	...
	private setupKongLaoDrag(): void {
		...
		this.dragHandler.registerElement({
			id: "laozi-pin",
			getPos: () => ({
				x: this.laoziPos.x - LAOZI_SIZE.w / 2,
				y: this.laoziPos.y - LAOZI_SIZE.h / 2,
			}),
			setPos: (pos) => {
				this.laoziPos = {
					x: pos.x + LAOZI_SIZE.w / 2,
					y: pos.y + LAOZI_SIZE.h / 2,
				};
			},
			getSize: () => LAOZI_SIZE,
		});
	...
	private setupStakesClick(): void {
		...
		canvas.addEventListener("touchstart", this.boundTouchStakeHandler, {
			passive: false,
		});
	}
	...
	private setupBellTuneDrag(): void {
		...
		this.dragHandler.setMode("layer");
		this.dragHandler.onDrag((state: DragState) => {
			// Map absolute drag position to striker position
			this.strikerX = this.strikerBaseX + state.totalDelta.x;
		...
		this.dragHandler.onDragEnd(() => {
			this.strikerBaseX = this.strikerX;
		});
	}
	...
	private renderFrame(): void {
		...
		switch (this.frame) {
			case ZhouFrame.SAL_SUN_DIAL:
				this.renderSundial(ctx);
				break;
			case ZhouFrame.SAL_ORACLE_BONE:
				this.renderOracle(ctx);
				break;
			case ZhouFrame.SAL_TRIPOD:
				this.renderTripod(ctx);
				break;
			case ZhouFrame.SAL_KONG_LAO:
			...
			case ZhouFrame.SAL_STAMP_ZHAIZI:
				...
				break;
			case ZhouFrame.LI_BELLS_APPEAR:
				this.renderBellsAppear(ctx);
				break;
			case ZhouFrame.LI_TUNE_BELLS:
				this.renderTuneBells(ctx);
				break;
			case ZhouFrame.LI_RESONANCE:
				this.renderResonance(ctx);
				break;
			case ZhouFrame.LI_PEONY:
				this.renderPeony(ctx);
				break;
			case ZhouFrame.LI_STAMP:
				this.renderLiStamp(ctx);
				break;
			case ZhouFrame.WD_CONFUCIUS:
				this.renderConfucius(ctx);
				break;
			case ZhouFrame.WD_ALTAR:
				this.renderAltar(ctx);
				break;
			case ZhouFrame.WD_GREEN_OX:
				this.renderGreenOx(ctx);
				break;
			case ZhouFrame.WD_PULP:
				this.renderPulp(ctx);
				break;
	...
	private renderSundial(ctx: CanvasRenderingContext2D): void {
		...
		for (let i = 0; i < 5; i++) {
			...
			ctx.fillStyle =
				i === this.sandialPhase ? "#C8A65A" : "rgba(200,166,90,0.4)";
			ctx.beginPath();
	...
	private renderResonance(ctx: CanvasRenderingContext2D): void {
		...
		for (let i = 0; i < BELL_COUNT; i++) {
			const bx = BELLS_START_X + i * BELL_SPACING;
			const vibrate =
				Math.sin(this.frameTimer / 80 + i) * 2 * this.resonanceAlpha;
			ctx.beginPath();
			ctx.moveTo(bx, BELLS_Y - 30);
